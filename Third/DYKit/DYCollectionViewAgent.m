...
#import "DYCollectionViewAgent.h"

@implementation DYCollectionViewAgent
...
- (DYTableViewModule *)defaultCollectionModule
{
    return DY_LAZY(_defaultTableModule,({
        DYTableViewModule *module = [[DYTableViewModule alloc] init];
        module.reuseIdentifier = @"UICollectionViewCell";
        module.slotBlock = ^BOOL(NSIndexPath *indexPath, id model) {return YES;};
        module;
    }));
}
...
- (nonnull __kindof UICollectionViewCell *)collectionView:(nonnull UICollectionView *)collectionView cellForItemAtIndexPath:(nonnull NSIndexPath *)indexPath {
    ...
    if (self.getSectionData) {
        for (DYTableViewModule *module in self.tableModuleLists) {
            if (module.slotBlock(indexPath,self.getSectionData(self.data[indexPath.section],indexPath.section)[indexPath.row])) {
                cell = [collectionView dequeueReusableCellWithReuseIdentifier:module.reuseIdentifier forIndexPath:indexPath];
//                cell = [collectionView dequeueReusableCellWithIdentifier:module.reuseIdentifier];
                module.assemblyBlock(cell, self.getSectionData(self.data[indexPath.section],indexPath.section)[indexPath.row], indexPath);
                return cell;
            }
        }
...
//    cell = [tableView dequeueReusableCellWithIdentifier:self.defaultTableModule.reuseIdentifier];
    cell = [collectionView dequeueReusableCellWithReuseIdentifier:self.defaultCollectionModule.reuseIdentifier forIndexPath:indexPath];
    
    if (self.getSectionData) {
        self.defaultCollectionModule.assemblyBlock(cell, self.getSectionData(self.data[indexPath.section],indexPath.section)[indexPath.row], indexPath);
        return cell;
    } else {
        self.defaultCollectionModule.assemblyBlock(cell, self.data[indexPath.row], indexPath);
        return cell;
    }
...
{
    return self.canMoveItemAtIndexPath ? self.canMoveItemAtIndexPath(collectionView,indexPath) : false;
}

- (NSIndexPath *)collectionView:(UICollectionView *)collectionView targetIndexPathForMoveFromItemAtIndexPath:(NSIndexPath *)originalIndexPath toProposedIndexPath:(NSIndexPath *)proposedIndexPath
{
    return self.targetCollectionIndexPathForMoveFromRowAtIndexPath ? self.targetCollectionIndexPathForMoveFromRowAtIndexPath(collectionView,originalIndexPath,proposedIndexPath) : proposedIndexPath;
}

- (void)collectionView:(UICollectionView *)collectionView moveItemAtIndexPath:(NSIndexPath *)sourceIndexPath toIndexPath:(NSIndexPath *)destinationIndexPath
{
//    if (self.collectionMoveItemAtNSIndexPath) {
//       self.collectionMoveItemAtNSIndexPath(collectionView, sourceIndexPath, destinationIndexPath);
//    }
}
...
{
    return self.canHandleDropSession ? self.canHandleDropSession(collectionView, session) : false;
}
...
- (nonnull NSArray<UIDragItem *> *)collectionView:(nonnull UICollectionView *)collectionView itemsForBeginningDragSession:(nonnull id<UIDragSession>)session atIndexPath:(nonnull NSIndexPath *)indexPath {
    return self.itemsForBeginningDragSession ? self.itemsForBeginningDragSession(collectionView, session, indexPath) : @[];
}
