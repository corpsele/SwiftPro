...
@implementation UICollectionView (DYCollectionViewBinder)

#pragma 属性
DYSYNTH_DYNAMIC_PROPERTY_OBJECT(agent, setAgent, RETAIN, DYCollectionViewAgent *)
...
- (UICollectionView*)setSectionData:(GetSectionData)block{
...
- (DYTableViewModule*) assembly:(AssemblyBlock)block{
    return [self assembly:block withPlug:UICollectionViewCell.class];
}
...
- (DYTableViewModule*) assembly:(AssemblyBlock)assemblyBlock fromSlot:(SlotBlock)slotBlock{
    return [self assembly:assemblyBlock fromSlot:slotBlock withPlug:UICollectionViewCell.class];
}
...
- (void)agentInitialize{
    if (!self.agent) {
        self.agent = [[DYCollectionViewAgent alloc] init];
        self.dataSource = self.agent;
        self.delegate = self.agent;
//        self.dragDelegate = self.agent;
//        self.dropDelegate = self.agent;
        self.autoReload = YES;
...
- (UICollectionView*)setCanMoveItemAtIndexPath:(BOOLCollectionViewIndexPath)block{
    [self agentInitialize];
    self.agent.canMoveItemAtIndexPath = block;
    return self;
    
}

- (UICollectionView*)setCanHandleDropSession:(BOOLCanHandleDropSession)block
{
    [self agentInitialize];
    self.agent.canHandleDropSession = block;
    return self;
}

- (UICollectionView*)setItemsForBeginningDragSession:(UIDragItemItemsForBeginningDragSession)block
{
    [self agentInitialize];
    self.agent.itemsForBeginningDragSession = block;
    return self;
}

- (UICollectionView*)setNumberOfItemsInSection:(NSIntegerCollectionViewIndexPath)block{[self agentInitialize];self.agent.numberOfItemsInSection = block;return self;}

- (UICollectionView*)setTargetCollectionIndexPathForMoveFromRowAtIndexPath:(NSIndexPathUICollectionViewNSIndexPathNSIndexPath)block{[self agentInitialize];self.agent.targetCollectionIndexPathForMoveFromRowAtIndexPath = block;return self;}

- (void)setMoveItemAtNSIndexPath:(MoveItemAtNSIndexPath)block
{
    [self agentInitialize];
    self.agent.collectionMoveItemAtNSIndexPath = block;
}
...
- (RACSignal*)didSelectItemAtIndexPathSignal{
    [self agentInitialize];
    DDLogInfo(@"agent = %@", self.agent);
    return [self.agent rac_signalForSelector:@selector(collectionView:didSelectItemAtIndexPath:)];
}
...
- (RACSignal*)moveItemAtIndexPathSignal{
    [self agentInitialize];
    DDLogInfo(@"agent = %@", self.agent);
    RACSignal *signal = [self.agent rac_signalForSelector:@selector(collectionView:moveItemAtIndexPath:toIndexPath:)];
    return signal;
}
