...
//            let i = index;
    ...
    @objc func btnStart1Event(sender: UIButton) {
...
//            let data: Data = UserDefaults.standard.data(forKey: "1") ?? Data();
    ...
    @objc func btnPause1Event(sender: UIButton) {
        let request = requests.first;
        request?.suspend();
    }
    
    @objc func btnStop1Event(sender: UIButton) {
        let request = requests.first;
        request?.cancel();
        downloadCount -= 1;
    ...
    @objc func btnStart2Event(sender: UIButton) {
...
//            let data: Data = UserDefaults.standard.data(forKey: "2") ?? Data();
    ...
    @objc func btnPause2Event(sender: UIButton) {
        let request = requests.last;
        request?.suspend();
    }
    
    @objc func btnStop2Event(sender: UIButton) {
        let request = requests.last;
        request?.cancel();
        downloadCount -= 1;
