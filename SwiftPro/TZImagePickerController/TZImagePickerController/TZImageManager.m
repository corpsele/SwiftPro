...
/// Get photo bytes 获得一组照片的大小
- (void)getPhotosFloatBytesWithArray:(NSArray *)photos completion:(void (^)(NSString *totalBytes))completion {
    if (!photos || !photos.count) {
    ...
    for (NSInteger i = 0; i < photos.count; i++) {
        ...
        [[PHImageManager defaultManager] requestImageDataForAsset:model.asset options:options resultHandler:^(NSData *imageData, NSString *dataUTI, UIImageOrientation orientation, NSDictionary *info) {
            ...
            assetCount ++;
            if (assetCount >= photos.count) {
                NSString *bytes = [self getBytesFromDataLengthForKB:dataLength];
                if (completion) completion(bytes);
            }
        }];
    }
...
- (NSString *)getBytesFromDataLengthForKB:(NSInteger)dataLength {
    NSString *bytes = [NSString stringWithFormat:@"%0.0f",dataLength/1024.0];
    return bytes;
}
